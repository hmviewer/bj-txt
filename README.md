...
별풍선/후원 로그 TXT 파일에서 특정 BJ 이름, 별칭, 초성 검색어에 해당하는 메시지만 찾아 시간순으로 정리하는 Electron 기반 Windows/macOS 데스크톱 앱입니다. 모든 로그 분석, 복사, TXT/CSV 내보내기는 로컬 PC에서만 수행됩니다.
...
- `release/타임라인-정리기-1.0.6-x64.exe`
...
macOS용 DMG/ZIP 파일은 `release/` 폴더에 생성됩니다.

예상 출력:

- `release/타임라인-정리기-1.0.6-mac-arm64.dmg`
- `release/타임라인-정리기-1.0.6-mac-arm64.zip`

현재 macOS 빌드는 Apple Silicon용이며, ad-hoc 서명을 적용합니다. 정식 외부 배포 시에는 Apple Developer ID로 서명하고 notarization을 추가하는 것을 권장합니다.

macOS에서 `손상됨` 경고가 보이면 앱을 `Applications` 폴더로 옮긴 뒤 아래 명령을 실행하세요.

```bash
xattr -cr "/Applications/Timeline Sorter.app"
```
...
src/
  ...
  features/
    ...
    sample/            개발용 샘플 로그
  styles/              Windows/macOS 생산성 앱 스타일
...
TXT 저장과 복사는 검색 결과에서 메시지 구간을 제외한 타임라인 라인만 줄바꿈으로 연결합니다.

예를 들어 아래 원문은:

```text
[ 00:15:28 ] [DL]윤파이. 1040 | "달리"
```

복사/TXT 저장 시 아래처럼 출력됩니다.

```text
[ 00:15:28 ] [DL]윤파이. 1040
```
...
```text
https://github.com/hmviewer/bj-txt/releases/tag/v1.0.6
```
